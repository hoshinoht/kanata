...
- **Output cap:** a chat route may declare `max_output_tokens` (1 to 1,048,576, and no more than `context_tokens` when both are set). Kanata shows it on `/v1/models` as `kanata.max_output_tokens` and rejects a larger `max_tokens`/`max_completion_tokens` with `400 invalid_request` naming that field. It doesn't fill in a default when the client omits one.
